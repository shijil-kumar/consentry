...
/** The columns this probe selects from credit_ledger. */
type LedgerRow = {
  delta_paise: number; reason: string | null;
  ref: string | null; created_at: string | null;
};
...
(async () => {
  ...
  for (const w of wallets ?? []) {
    ...
    const sum = (led ?? []).reduce((a, r: LedgerRow) => a + Number(r.delta_paise), 0);
    console.log(`org ${w.org_id.slice(0,8)}  wallet=${w.balance_paise}  ledger=${sum}  ${sum === Number(w.balance_paise) ? "OK" : "*** MISMATCH ***"}`);
    for (const r of led ?? []) {
      console.log(`   ${String(r.reason).padEnd(12)} ${String(r.delta_paise).padStart(9)}  ref=${String(r.ref ?? "-").slice(0, 8)}  ${r.created_at?.slice(0, 16)}`);
    }
  }
})();
